...
void deleteInQueue(Queue *Q, int id);
...
void completionTime(Process *p, int n)
{
    ...
    while (1)
    {
        ...
        if (temp != NULL)
        {
            if(temp->p->brt >=quantum){
                temp->p->ct = temp->p->ct + quantum;
                b[rr->p->pid]-=quantum;
                cpuTime+=quantum=temp->p->ct;
            }
            else{
                temp->p->ct = temp->p->ct + b[rr->p->pid];
                b[rr->p->pid]=0;
                cpuTime+=quantum=temp->p->ct;
            }
            deleteInQueue(readyq, temp->p->pid);
...
void gantChart(Process *p, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        printf("\nP[%d]\t%d\t%d\t%d\t%d\t%d\t%d", p[i].pid, p[i].art, p[i].brt, p[i].pr, p[i].ct, p[i].tt, p[i].wt);
    }
    printf("\n");
}